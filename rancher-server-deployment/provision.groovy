...
inlineInventory {
  node id: 'haproxy',   host: '192.168.55.10', username: 'vagrant', keyfile: keypath('haproxy'),   tags: [role: 'haproxy']
  node id: 'rancher-1', host: '192.168.55.11', username: 'vagrant', keyfile: keypath('rancher-1'), tags: [role: 'rancher']
  node id: 'rancher-2', host: '192.168.55.12', username: 'vagrant', keyfile: keypath('rancher-2'), tags: [role: 'rancher']
  node id: 'mysql',     host: '192.168.55.13', username: 'vagrant', keyfile: keypath('mysql'),     tags: [role: 'mysql']
}.provision {
  ...
  task name: 'run a mysql server', filter: {'role:mysql'}, actions: {
    ...
    info "waiting for mysql instance is up and running"
    retry count: 10, delay: 2000, actions: {
      assert canConnectTo(port: 3306, host: node.host)
    }
  }
  ...
  task name: 'run a couple of rancher servers', parallel: 2, filter: {'role:rancher'}, actions: {
    ...
    shell """
      ...
      --name=rancher-server rancher/server:v1.6.18 \
  ...
  task name: 'run an haproxy load balancer', filter: {'role:haproxy'}, actions: {
    ...
    directory user: 'root', target: '/etc/haproxy', mode: 600
    template(mode: 600, user: 'root') {
